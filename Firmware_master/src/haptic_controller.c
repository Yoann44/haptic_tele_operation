...
#define RECV_TORQUE_TIMEOUT 				0.2 //200ms
...
volatile uint32_t hapt_virtualWall = 1;
...
volatile float32_t hapt_receivedTorque = 0.0;
...
void hapt_Init(void)
{
    ...
    comm_monitorFloat("Receive torque [Nm]", (float32_t*)&hapt_receivedTorque, READONLY);
...
void sendPosition();
bool receiveTorque(float* output);
...
void hapt_Update()
{
	...
	static float32_t dt_send_position = SEND_TORQUE_PERIOD;
	if(dt_send_position > SEND_TORQUE_PERIOD) {
		sendPosition();
		dt_send_position = 0;
	}
	else {
		dt_send_position += ((float32_t)cbt_GetHapticControllerPeriod()) / 1000000.0f;
	}

	float recv_torque;
	static float32_t dt_recv_torque = RECV_TORQUE_TIMEOUT;
	if(receiveTorque(&recv_torque)) {
		hapt_receivedTorque = recv_torque;
		dt_recv_torque = 0;
	}
	else {
		dt_recv_torque += ((float32_t)cbt_GetHapticControllerPeriod()) / 1000000.0f;
	}

	if(dt_recv_torque < RECV_TORQUE_TIMEOUT) {
		hapt_motorTorque += hapt_receivedTorque;
	}
...
void sendPosition() {
	char buffer[32];
	sprintf(buffer, "#%.2f$", hapt_paddleAngle);
